...
class tx_f2facebook_bepreview {
		...
		public function renderPreviewContent_preProcess ($row, $table, &$alreadyRendered, &$reference) {
				if ($row['CType'] === 'list' && preg_match('/f2facebook_/',$row['list_type'])) {
		...
		protected function preview($row,$type) {
				switch ($type) {
					case 'f2facebook_pi1':
						$content = '<img src="../../../'.t3lib_extMgm::extRelPath('f2facebook').'Resources/Public/Icons/plugin-preview.png" />';
						break;
					default:
						$content = '<img src="../../../'.t3lib_extMgm::extRelPath('f2facebook').'Resources/Public/Icons/plugin-preview.png" />';
						break;
